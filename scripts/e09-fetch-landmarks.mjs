...
const results = [];
let n = 0;
for (const it of WORLD_LANDMARKS_E09) {
  n++;
  const ext = "jpg";
  const outPath = `${DEST}/${it.slug}.${ext}`;
  if (existsSync(outPath)) { results.push({ slug: it.slug, name: it.name, status: "SKIP_EXISTS" }); console.error(`${n}/71 ${it.slug}: SKIP`); continue; }
  try {
    const found = await imageUrl(it.name);
    await sleep(1800);
    if (!found) { results.push({ slug: it.slug, name: it.name, status: "NO_IMAGE" }); console.error(`${n}/71 ${it.slug}: NO_IMAGE`); continue; }
    const dl = await download(found.url, outPath);
    await sleep(1800);
    if (!dl.ok) { results.push({ slug: it.slug, name: it.name, status: "DL_FAIL", reason: dl.reason, wikiTitle: found.title, url: found.url }); console.error(`${n}/71 ${it.slug}: DL_FAIL ${dl.reason}`); continue; }
    results.push({ slug: it.slug, name: it.name, status: "OK", size: dl.size, wikiTitle: found.title, url: found.url });
...
console.log(`\nOK ${ok.length} / ${WORLD_LANDMARKS_E09.length}`);
if (bad.length) { console.log("NEED FALLBACK:"); for (const b of bad) console.log(` - ${b.slug} (${b.name}): ${b.status} ${b.reason || ""}`); }
writeFileSync("scripts/e09-fetch-report.json", JSON.stringify(results, null, 2), "utf8");
