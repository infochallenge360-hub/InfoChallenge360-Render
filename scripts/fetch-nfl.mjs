// Fetches NFL team badges from TheSportsDB -> public/nfl79/<slug>.png
// Usage: node scripts/fetch-nfl.mjs
import { writeFileSync, existsSync, mkdirSync, statSync } from "node:fs";
import { pathToFileURL } from "node:url";

const KEY = "123";
const DEST = "public/nfl79";
if (!existsSync(DEST)) mkdirSync(DEST, { recursive: true });
const mod = await import(pathToFileURL("src/Quiz/nflE79Data.js").href);
...
for (const t of teams) {
  ...
  try {
    ...
    let matches = (j.teams || []).filter((x) => x.strLeague === "NFL" && x.strBadge);
    if (!matches.length) { missing.push(t.slug + " (no NFL result)"); await sleep(2200); continue; }
    if (matches.length > 1) ambiguous.push(`${t.slug} -> picked "${matches[0].strTeam}"; ${matches.length} NFL matches`);
...
console.log(`\nNFL badges: ${ok.length}/${teams.length} fetched.`);
