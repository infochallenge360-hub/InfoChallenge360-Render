...
const report = JSON.parse(readFileSync("out/e37-instruments-fetch-report.json", "utf8"));

// These 2 were fixed via manual one-off commands after the tracked fetch script got them wrong —
// override with the actual final filenames used.
const OVERRIDES = {
  "guitar": "Old_Gypsy_Acoustic_Guitar_(43120253700).jpg",
  "banjo": "Banjo_(AM_1998.60.131-1).jpg",
};
...
const items = report.filter((r) => r.status === "ok").map((r) => ({ slug: r.slug, filename: OVERRIDES[r.slug] || filenameFromUrl(r.url) }));
...
writeFileSync("out/e37-instruments-credits.json", JSON.stringify(out, null, 2));
