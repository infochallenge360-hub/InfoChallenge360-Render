// يجلب صور الهواتف من Wikipedia pageimages API — تسلسلي (ليس متوازي) لتفادي throttling.
// الاستخدام: node scripts/fetch-phones-images.mjs
import { PHONES_E65 } from "../src/Quiz/phonesE65Data.js";
import { writeFileSync, existsSync, statSync } from "node:fs";

const UA = "InfoChallenge360QuizBot/1.0 (educational quiz; contact: shehaltoughtalat@gmail.com)";
const DEST = "public/phones";
...
console.log(`\n${ok}/${PHONES_E65.length} images ok.`);
if (fails.length) {
  console.log(`\n${fails.length} FAILED:`);
  fails.forEach((f) => console.log(`  ${f.slug} (wiki: "${f.wiki}") — ${f.reason}`));
}
writeFileSync("out/_phones_fetch_report.json", JSON.stringify(results, null, 2));
