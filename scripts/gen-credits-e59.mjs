...
const dataMod = await import(pathToFileURL("src/Quiz/palacesE59Data.js").href);
const DATA = dataMod.default;
const report = JSON.parse(readFileSync("out/e59-palaces-fetch-report.json", "utf8"));
const bySlug = Object.fromEntries(report.map((r) => [r.slug, r]));

const levels = ["easy", "medium", "hard", "impossible"];
let out = `Info Challenge 360 — Episode 59 "Guess the Palace" — Photo Credits
======================================================================
All palace photographs sourced from Wikipedia/Wikimedia Commons, license-filtered
...
writeFileSync("D:/InfoChallenge360-Channel/02_Episodes/E59_Palaces/seo/InfoChallenge360-E59-Palaces-PHOTO-CREDITS.txt", out.trim() + "\n");
console.log("done");
