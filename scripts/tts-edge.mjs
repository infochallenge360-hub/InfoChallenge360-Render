...
// الاستخدام: node scripts/tts-edge.mjs <dataFile> <prefix> <nameField> [voice]
...
const [dataFile, prefix, nameField = "name", voice = "en-US-GuyNeural"] = process.argv.slice(2);
if (!dataFile || !prefix) { console.log("usage: node tts-edge.mjs <dataFile> <prefix> <nameField> [voice]"); process.exit(1); }
...
for (const it of arr) {
  ...
  try {
    sh(`python -m edge_tts --voice ${voice} --text ${JSON.stringify(text)} --write-media ${TMP}`);
