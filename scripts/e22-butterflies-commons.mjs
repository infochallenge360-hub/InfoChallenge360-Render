...
import { BUTTERFLIES } from "../src/Quiz/butterfliesData.js";
...
const list = BUTTERFLIES.filter((f) => TARGETS.includes(f.slug));
...
async function commons(query, slug) {
  ...
  for (const p of pages) {
    ...
    if (/(illustration|drawing|botanical|herbarium|caterpillar|larva|pupa|chrysalis|cocoon|egg|specimen|pinned|mounted|plate|seed|leaf|distribution|map|diagram|poster|painting|print|artwork|\bart\b|stamp|coin|sketch|engraving|lithograph|waterhouse|\becho\b|myth|vanessa|butterfly|moth|\bbee\b|\bfly\b|insect|beetle|hoverfly|bombus|wasp|logo|label)/i.test(p.title)) continue;
    const rr = await fetch(ii.thumburl || ii.url, { headers: { "User-Agent": UA } });
    const buf = Buffer.from(await rr.arrayBuffer());
    if (!valid(buf)) continue;
    writeFileSync(`public/butterflies/${slug}.jpg`, buf);
...
for (const f of list) {
  let done = false;
  for (const q of [`${f.sci} butterfly`, `${f.name} butterfly`, `${f.sci} moth`, `${f.sci}`]) {
