...
const list = SNAKES.filter((f) => TARGETS.includes(f.slug));
...
async function commons(query, slug) {
  ...
  for (const p of pages) {
    ...
    if (/(illustration|drawing|botanical|herbarium|specimen|pinned|mounted|plate|distribution|map|diagram|poster|painting|print|artwork|art|stamp|coin|sketch|engraving|lithograph|skull|skeleton|bone|preserved|larva|egg|logo|label|watermark|genter)/i.test(p.title)) continue;
    const rr = await fetch(ii.thumburl || ii.url, { headers: { "User-Agent": UA } });
    const buf = Buffer.from(await rr.arrayBuffer());
    if (!valid(buf)) continue;
    writeFileSync(`public/snakes/${slug}.jpg`, buf);
...
for (const f of list) {
  let done = false;
  for (const q of [`${f.sci}`, `${f.name} snake`, `${f.sci} snake`, `${f.name} ${f.sci}`]) {
