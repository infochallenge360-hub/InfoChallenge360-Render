// Resolve the actual Commons/local file used per E76 airplane slug and report its license.
...
const dataMod = await import("file://" + process.cwd().replace(/\\/g, "/") + "/src/Quiz/airplanesE76Data.js");
const items = dataMod.AIRPLANES_E76;

const results = [];
for (const item of items) {
  let file = await pageimageTitle(item.wiki);
...
writeFileSync("out/e76-credits.json", JSON.stringify(results, null, 2));
