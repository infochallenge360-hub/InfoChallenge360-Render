...
// TSV audit trail
writeFileSync("scripts/e19-credits.tsv", rows.map((r) => `${r.slug}\t${r.name}\t${r.license}\t${r.attribution}`).join("\n") + "\n", "utf8");
...
writeFileSync("scripts/e19-attribution-block.txt", `PHOTO CREDITS (iNaturalist, CC BY) — include in video description:\n\n${block}\n`, "utf8");
